...
// RESTORED (was accidentally deleted in commit c504f23 → production 404).
// Files are now persisted to Supabase Storage (see src/lib/storage.ts) —
// no more base64 data URIs bloating the database.
//
// Limits are Vercel-safe: serverless request body hard cap is ~4.5MB, so the
// server accepts at most 6MB images / 4MB videos; the client additionally
// compresses big images and uploads one file per request (see
// src/lib/upload-client.ts) so real payloads never approach the cap.
...
export async function POST(req: NextRequest) {
  ...
  // PRECISE auth classification — each case gets its own code:
